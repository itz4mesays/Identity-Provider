...
router.post('/sso/login', async (req: Request, res: Response) => {
  try {
    ...
    console.log(`Completed creating login response`);



    // If you need to return the Base64-encoded, URL-encoded version:
    // const base64Response = Buffer.from(response).toString('base64');
    // const urlEncodedResponse = encodeURIComponent(base64Response);
    // res.redirect(`http://localhost:8000/saml/acs?SAMLResponse=${urlEncodedResponse}`);

    // Current implementation returns raw XML
